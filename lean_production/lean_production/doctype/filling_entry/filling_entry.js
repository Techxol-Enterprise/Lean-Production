...
frappe.ui.form.on("Filling Entry", {
	setup: function(frm) {
		// Filter finished_good_item to Finished Goods branch
		...
		// Filter BOMs to only submitted, active BOMs for the selected item
		frm.set_query("bom_no", function() {
			let filters = { is_active: 1, docstatus: 1 };
			if (frm.doc.finished_good_item) {
				filters.item = frm.doc.finished_good_item;
			}
	...
	finished_good_item: function(frm) {
		if (frm.doc.finished_good_item) {
			frappe.db.get_value("BOM", { item: frm.doc.finished_good_item, is_active: 1, is_default: 1, docstatus: 1 }, "name", function(r) {
				if (r && r.name) {
					frm.set_value("bom_no", r.name);
				} else {
					frappe.db.get_value("BOM", { item: frm.doc.finished_good_item, is_active: 1, docstatus: 1 }, "name", function(r2) {
						if (r2 && r2.name) {
	...
	good_qty: function(frm) {
		frm.doc.cartons_produced = frm.doc.good_qty;
		calculate_and_populate_materials(frm);
	},
